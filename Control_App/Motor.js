...
class Motor extends Placeable {
  ...
  updateWindingCoordinates() {
    // U winding
    this.u.vertex1.x = this.x - 4 * spacing;
    this.u.vertex1.y = this.y - 7 * spacing;

    this.u.vertex2.x = this.x - 4 * spacing;
    this.u.vertex2.y = this.y + 7 * spacing;

    // V winding
    this.v.vertex1.x = this.x;
    this.v.vertex1.y = this.y - 7 * spacing;

    this.v.vertex2.x = this.x;
    this.v.vertex2.y = this.y + 7 * spacing;

    // W winding
    this.w.vertex1.x = this.x + 4 * spacing;
    this.w.vertex1.y = this.y - 7 * spacing;

    this.w.vertex2.x = this.x + 4 * spacing;
    this.w.vertex2.y = this.y + 7 * spacing;

    // console.log(
    //   "in motor.getWindings u: " + this.u,
    //   "v: " + this.v,
    //   "w: " + this.w
    // );
  }
  ...
  getWindings() {
    // Used in Simulator.js in function
    // initializeSimulation()
    return [this.u, this.v, this.w];
  }

  updateMotor() {
    // Used in PlaceDevice.js under motorUpdateContacts()
    // motorUpdateContacts() used in Controller.js
    // setFeed() is in simulator.js in fn filterMotorWindings()
