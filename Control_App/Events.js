...
function mousePressed() {
  ...
  if (state == "PLACE_DEVICE" && !done) {
    ...
    if (currentAuxiliary && !currentAuxiliary.isPlaced()) {
      for (const contactor of contactors) {
        if (contactor.isMouseOver(x, y)) {
          currentAuxiliary.snapToBase(contactor);
          currentAuxiliary.assignContacts();
  ...
  if (state == "SIMULATE" && !done) {
    ...
    for (const selector of selectorSwitches) {
      if (selector.isMouseOver(x, y)) {
...
function doubleClicked() {
  ...
  for (let i = timers.length - 1; i >= 0; i--) {
    ...
    if (timer.isMouseOver(position.x, position.y)) {
      timer.editTimeout();
      return false;
    }
  }
