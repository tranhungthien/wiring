...
function dropDown(canvas) {
  ...
  let jogButtonOption = createDiv("Jog Button &emsp;");
  jogButtonOption.style("padding", "5px");
  jogButtonOption.mousePressed(() => {
    state = "JOG_BUTTON";
    done = false;
    contextMenu.hide();
  });
  jogButtonOption.mouseOver(() =>
    jogButtonOption.style("background-color", "#e0e0e0")
  );
  jogButtonOption.mouseOut(() =>
    jogButtonOption.style("background-color", "#fff")
  );
  contextMenu.child(jogButtonOption);
  //-----------------------------------------------

  let selectorSwitchOption = createDiv("SPDT Switch");
  selectorSwitchOption.style("padding", "5px");
  selectorSwitchOption.mousePressed(() => {
    state = "SELECTOR_SWITCH";
    done = false;
    contextMenu.hide();
  });
  selectorSwitchOption.mouseOver(() =>
    selectorSwitchOption.style("background-color", "#e0e0e0")
  );
  selectorSwitchOption.mouseOut(() =>
    selectorSwitchOption.style("background-color", "#fff")
  );
  contextMenu.child(selectorSwitchOption);
  //-----------------------------------------------
  ...
  let thermalOverLoadOption = createDiv("TOL");
  thermalOverLoadOption.style("padding", "5px");
  thermalOverLoadOption.mousePressed(() => {
    state = "THERMAL_OVERLOAD";
    done = false;
    contextMenu.hide();
  });
  thermalOverLoadOption.mouseOver(() =>
    thermalOverLoadOption.style("background-color", "#e0e0e0")
  );
  thermalOverLoadOption.mouseOut(() =>
    thermalOverLoadOption.style("background-color", "#fff")
  );
  contextMenu.child(thermalOverLoadOption);
  //-----------------------------------------------

  let auxiliaryOption = createDiv("Auxiliary");
  auxiliaryOption.style("padding", "5px");
  auxiliaryOption.mousePressed(() => {
    if (contactors.length >= auxiliaries.length + 1) {
      state = "AUXILIARY";
      done = false;
    }
    contextMenu.hide();
  });
  auxiliaryOption.mouseOver(() =>
    auxiliaryOption.style("background-color", "#e0e0e0")
  );
  auxiliaryOption.mouseOut(() =>
    auxiliaryOption.style("background-color", "#fff")
  );
  contextMenu.child(auxiliaryOption);
  //-----------------------------------------------

  let terminalBlockOption = createDiv("Terminal Block");
  terminalBlockOption.style("padding", "5px");
  terminalBlockOption.mousePressed(() => {
    state = "TERMINAL_BLOCK";
    done = false;
    contextMenu.hide();
  });
  terminalBlockOption.mouseOver(() =>
    terminalBlockOption.style("background-color", "#e0e0e0")
  );
  terminalBlockOption.mouseOut(() =>
    terminalBlockOption.style("background-color", "#fff")
  );
  contextMenu.child(terminalBlockOption);
  //-----------------------------------------------

  let textLabelOption = createDiv("Text Label &emsp;&emsp;<b>l</b>");

  textLabelOption.style("padding", "5px");

  textLabelOption.mousePressed(() => {
    state = "TEXT_LABEL";
    done = false;
    contextMenu.hide();
  });
  textLabelOption.mouseOver(() =>
    textLabelOption.style("background-color", "#e0e0e0")
  );
  textLabelOption.mouseOut(() =>
    textLabelOption.style("background-color", "#fff")
  );
  contextMenu.child(textLabelOption);
